...
<div class="card card-default">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('inventories.update', $inventory->id) }}">
            ...
            <div class="mb-3">
                <label>Name</label>
                <input type="text" name="name" value="{{ old('name', $inventory->name) }}" class="form-control"
                    required>
            </div>
            ...
            <div class="mb-3">
                <label>Amount</label>
                <input type="number" step="0.01" name="amount" value="{{ old('amount', $inventory->amount) }}"
                    class="form-control" required>
            </div>

            <div class="mb-3">
                <label>Voucher ID</label>
                <input type="text" name="voucher_id" value="{{ old('voucher_id', $inventory->voucher_id) }}"
                    class="form-control">
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description"
                    class="form-control">{{ old('description', $inventory->description) }}</textarea>
            </div>

            <div class="mb-3">
                <label>Status</label>
                <select name="status" class="form-control">
                    <option value="inuse" {{ old('status', $inventory->status) == 'inuse' ? 'selected' : '' }}>In
                        Use</option>
                    <option value="repaired" {{ old('status', $inventory->status) == 'repaired' ? 'selected' : '' }}>
                        Repaired</option>
                    <option value="replaced" {{ old('status', $inventory->status) == 'replaced' ? 'selected' : '' }}>
                        Replaced</option>
                </select>
            </div>
